...
- **Vision OCR:** 이미지 형태의 문제를 텍스트 및 수식으로 변환하고 문제의 의도를 파악합니다.
- **이미지 처리:** PDF 및 이미지 파일에서 문제를 추출하고 전처리를 수행합니다.

### 2. RAG 영역
- **Semantic Search:** pgvector를 활용하여 강의 자료 및 전공 서적에서 관련 개념을 검색합니다.
- **Context-Aware:** 검색된 지식을 바탕으로 환각 현상을 최소화한 답변을 생성합니다.

### 3. 멀티 에이전트 워크플로우 (Subgraphs)
- **Explain:** 복잡한 이공계 개념을 단계별로 상세히 설명합니다.
- **Solve:** 문제 풀이 과정을 논리적으로 전개하며 최적의 해법을 제시합니다.
- **Solution PDF:** LaTeX 수식과 차트가 포함된 고품질 해설 PDF를 자동으로 생성합니다.
- **Check:** 사용자가 작성한 풀이나 정답의 논리적 오류를 검토합니다.
- **Variant:** 학습한 문제를 바탕으로 숫자나 조건을 변형한 유사 문항을 생성합니다.

### 4. 코드 인터프리터 (E2B)
- 복잡한 수치 계산이나 시뮬레이션이 필요한 경우 Python 코드를 작성하고 실행하여 정확한 결과를 도출합니다.
